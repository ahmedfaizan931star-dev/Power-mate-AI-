...
class AppUsageStatsManager(private val context: Context) {
    ...
    fun topAppsSince(hours: Int = 24, limit: Int = 10): List<AppUsageEntry> {
        ...
        // Fallback only kicks in when the precise event log gives us nothing at all for a
        // package (e.g. events were pruned by the OS). queryUsageStats() buckets are whole-day
        // aggregates and are NOT clipped to our query window, so they must never be blended
        // with (or allowed to inflate) numbers that already came from real events.
        val fallback24h = collectFallbackUsageStats(start24h, end)
        val fallbackToday = collectFallbackUsageStats(startToday, end)

        val packageNames = (last24h.keys + today.keys + fallback24h.keys + fallbackToday.keys)
        ...
        val merged = packageNames.mapNotNull { packageName ->
            ...
            // Only trust the daily-bucket fallback when we have zero event-based data for this
            // package in this window — using it as a ceiling via max() previously let inflated,
            // unclipped daily totals override accurate event-derived durations.
            val last24hMs = last24hStats?.foregroundMs?.takeIf { it > 0L }
                ?: fallback24h[packageName]?.foregroundMs
                ?: 0L
            val todayMs = todayStats?.foregroundMs?.takeIf { it > 0L }
                ?: fallbackToday[packageName]?.foregroundMs
                ?: 0L
            ...
            val lastUsed = maxOf(
                last24hStats?.lastUsed ?: 0L,
                todayStats?.lastUsed ?: 0L,
                fallback24h[packageName]?.lastUsed ?: 0L,
                fallbackToday[packageName]?.lastUsed ?: 0L
            )
            val launches = max(last24hStats?.launchCount ?: 0, todayStats?.launchCount ?: 0)
            val category = categoryLabel(appInfo, packageName)
            ...
            AppUsageEntry(
                ...
                drainHint = buildDrainHint(foregroundMs, launches, category)
            )
        ...
        while (events.hasNextEvent()) {
            ...
            when {
                isForegroundEvent(event.eventType) -> {
                    // If the package is already considered "active" (e.g. the user moved
                    // between two activities/tabs inside the same app), keep the earlier
                    // start time instead of resetting it — resetting here was silently
                    // dropping the time already spent in foreground before this event.
                    activeStarts.getOrPut(packageName) { event.timeStamp }
                ...
                isBackgroundEvent(event.eventType) -> {
                    ...
                    // Guard against out-of-order or duplicate events producing a negative or
                    // implausibly long single session, which would otherwise skew totals.
